import 'package:gerenciador_gastos_v2/services/db_service.dart';
...
class DatabaseController {
  ...
  static final _instance = DatabaseController._();
  DatabaseController._();
  factory DatabaseController.instance() => _instance;
  ...
  Future<void> selectExpensesByDate({
    ...
    expenses = database.selectExpenseByDate(
      groupID: groupID,
      month: month,
      year: year,
    );
  ...
  Future<void> selectExpensesByGroupAndPaymentMethod({
    ...
    if (day != null && month != null) {
      ...
      for (int i = 0; i < length; i++) {
        if (int.parse(expensesWithoutFuture[i].date.substring(3, 5)) ==
            monthAsInt) {
          if (int.parse(expensesWithoutFuture[i].date.substring(0, 2)) <
              dayAsInt) {
            indexToRemove.add(i);
          }
        } else {
  ...
  Future<void> updateExpense({
    required ExpenseWrite expenseData,
