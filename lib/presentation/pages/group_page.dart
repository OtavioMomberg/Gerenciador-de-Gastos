...
import 'package:gerenciador_gastos_v2/presentation/widgets/expense_card.dart';
...
class _GroupPageState extends State<GroupPage>
  ...
  void initState() {
    super.initState();
    widget.groupService.initControllers();
  }
          ...
          IconButton(
            onPressed: () async {
              ...
              if (await deleteProcess(message: message)) {
                await widget.db.deleteGroup(groupID: widget.groupID);
      ...
      body: Container(
        ...
        child: SafeArea(
          child: Column(
            children: <Widget>[
              IgnorePointer(
                ignoring: widget.groupService.isExpenseSelected ? false : true,
                child: AnimatedOpacity(
                  duration: Duration(milliseconds: 300),
                  curve: Curves.easeInOut,
                  opacity: widget.groupService.isExpenseSelected ? 1.0 : 0.0,
                  child: Button(
                    ...
                    function: () async {
                      ...
                      if (await deleteProcess(message: message)) {
                        await widget.db.deleteSelectedExpenses(
                          expenseID: widget.groupService.indexList,
                        );
                        showResponse(message: "Gastos removidos com sucesso!");
                        widget.groupService.updateIsExpenseSelected();
                        widget.groupService.indexList.clear();
                      }
                    },
                  ),
                ),
  ...
  Future<void> filter() async {
    await showDialog(
      ...
      builder: (_) => AlertDialog(
        backgroundColor: AppThemes.color3,
        title: Center(child: const Text("Filtro", style: AppThemes.textStyle)),
        content: Column(
          mainAxisSize: .min,
          spacing: 10,
          children: <Widget>[
            TextInput(
              controller: widget.groupService.month,
              textHint: "Mês",
              inputType: TextInputType.number,
            ),
            TextInput(
              controller: widget.groupService.year,
              textHint: "Ano",
              inputType: TextInputType.number,
            ),
            const SizedBox(height: 10),
  ...
  void closeDialog() {
    widget.groupService.month.clear();
    widget.groupService.year.clear();
    if (!mounted) {
      return;
    }
    Navigator.pop(context);
  ...
  void thenFunction({bool? response}) async {
    await widget.db.selectExpensesByGroup(groupID: widget.groupID);
    if (!mounted) {
      return;
    }
    setState(() {});
  ...
  @override
  void dispose() {
    widget.groupService.month.dispose();
    widget.groupService.year.dispose();
    super.dispose();
  }
}
